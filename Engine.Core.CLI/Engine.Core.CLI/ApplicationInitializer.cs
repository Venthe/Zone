...
namespace Engine.Core.CLI
{
    public class ApplicationInitializer
    {
        ...
        private static void InitializeScripts(ref Game game_)
        {
            var scripts = new ScriptRepository();
            Script.Game = game_;

            // TODO: Fixme
            scripts.Add(new Script("sexGod", (game, scriptsLibrary, arguments) => false));

            scripts.Add(new Script("objSexRating", (game, scriptsLibrary, arguments) =>
            {
                var charisma = game.Player.Attributes.GetById(Attribute.Charisma).Value;
                var endurance = game.Player.Attributes.GetById(Attribute.Endurance).Value;
                var agility = game.Player.Attributes.GetById(Attribute.Agility).Value;
                var strength = game.Player.Attributes.GetById(Attribute.Strength).Value;
                var kamaSutraMaster = game.Player.Perks.GetById("KamaSutraMaster")?.Rank > 0 ? 2 : 0;
                var sexAppeal = game.Player.Perks.GetById("SexAppeal")?.Rank > 0 ? 1 : 0;
            ...
            scripts.Add(new Script("weaponFinalDamage", (game, scriptsLibrary, arguments) =>
            {
                // FIXME 
            ...
            scripts.Add(new Script("baseSkillValue", (game, scriptsLibrary, arguments) =>
            {
                var statValue = arguments ?? 5;
                var luckValue = game.Player.Attributes.GetById(Attribute.Luck).Value;
                return 2 + (2 * (int)statValue) + (luckValue / 2);
            }));

            scripts.Add(new Script(DerivedAttribute.ActionPoints, (game, scriptsLibrary, arguments) => 65 + (game.Player.Attributes.GetById(Attribute.Agility).Value * 3)));

            scripts.Add(new Script(DerivedAttribute.CarryWeight, (game, scriptsLibrary, arguments) => 150 + (game.Player.Attributes.GetById(Attribute.Strength).Value * 10)));

            scripts.Add(new Script(DerivedAttribute.CompanionNerve, (game, scriptsLibrary, arguments) => (int)(0.05 * game.Player.Attributes.GetById(Attribute.Charisma).Value)));

            scripts.Add(new Script(DerivedAttribute.CriticalChance, (game, scriptsLibrary, arguments) => (int)(0.01 * game.Player.Attributes.GetById(Attribute.Luck).Value)));

            scripts.Add(new Script(DerivedAttribute.HitPoints, (game, scriptsLibrary, arguments) => (game.Player.Level * 5) + 95 + (20 * game.Player.Attributes.GetById(Attribute.Endurance).Value)));

            scripts.Add(new Script(DerivedAttribute.MeleeDamage, (game, scriptsLibrary, arguments) => (int)(0.5 * game.Player.Attributes.GetById(Attribute.Strength).Value)));

            scripts.Add(new Script(DerivedAttribute.PoisonResistance, (game, scriptsLibrary, arguments) => (5 * game.Player.Attributes.GetById(Attribute.Endurance).Value) - 5));

            scripts.Add(new Script(DerivedAttribute.RadiationResistance, (game, scriptsLibrary, arguments) => (2 * game.Player.Attributes.GetById(Attribute.Endurance).Value) - 2));

            scripts.Add(new Script(DerivedAttribute.ReloadSpeed, (game, scriptsLibrary, arguments) =>
            {
                float value = game.Player.Attributes.GetById(Attribute.Agility).Value;
                return (int)(value > 5 ? 0.1 * (value - 5) : 0);
            }));

            scripts.Add(new Script(DerivedAttribute.SkillRate, (game, scriptsLibrary, arguments) => (int)(10 + (0.5 * game.Player.Attributes.GetById(Attribute.Intelligence).Value))));

            scripts.Add(new Script(DerivedAttribute.UnarmedDamage, (game, scriptsLibrary, arguments) =>
            {
                return (int)(0.5 + (game.Player.Skills.GetById(Skill.Unarmed).Value / 20));
            }));

            scripts.Add(new Script("testAddPerk", (game, scriptsLibrary, perkName) =>
            {
                game.Player.Perks.Add((string) perkName);
                return null;
            }));
        }
        ...
        private static void InitializeDerivedAttributesTranslationsRepository()
        {
            ...
            translations.Add(new BaseTranslation(DerivedAttribute.Sleep, "Sleep", "A meter that tells you how long the player has been awake and when the player needs sleep. (Hardcore mode)"));
