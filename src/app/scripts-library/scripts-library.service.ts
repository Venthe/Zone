...
import { AppState, GlobalState } from '../state/app.reducer';
...
export class ScriptsLibraryService {
    ...
    constructor(private readonly scripts: ScriptsProvider, private readonly store: Store<GlobalState>) {
        ...
        store.select(selectAppFeature).subscribe(state => {
            console.log(state)
            return this.globalVariables = state;
        });
    }
    ...
    execute(scriptName: string, props: object & { globalVariablesOverride?: object } = {}) {
        ...
        console.log("Executing script.", `scriptName:${scriptName}`, "props:", props, "globalVariables:", this.globalVariables, "result:", result)
        return result;
    }
